...
// 导致锚点每轮漂移；而首个 block（v2.1.181+ 的 attribution 块）会话内稳定。
...
// 未指定返回 ""。这是 Claude/OpenAI Chat 获取显式会话 ID 的通道
// （两者请求体均无自由 metadata 字段）。
...
// resolveChainRoot 沿 previous_response_id 向上取链根 response 的 ID（O(1)，父已存根）。
...
// 消息列表（previous_response_id 展开的历史 + instructions + input），由
// handleOpenAIResponses 传入。r 可为 nil。
